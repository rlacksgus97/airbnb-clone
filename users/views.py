...
def github_login(request):
    ...
    return redirect(f"https://github.com/login/oauth/authorize?clien_id={client_id}&redirect_uri={redirect_uri}&scope=read:user")
            ...
            if error is not None:
                ...
                if username is not None:
                    ...
                    try:
                        ...
                        user = models.User.objects.create(email=email, first_name=name, username=email, bio=bio, login_method=models.User.LOGIN_GITHUB, email_verified = True)
...
def kakao_callback(request):
    try:
        ...
        access_token = token_json.get("access_token")
        profile_request = requests.get("https://kapi.kakao.com/v1/user/me", headers={"Authorization": f"Bearer {access_token}"},)
        profile_json = profile_request.json()
